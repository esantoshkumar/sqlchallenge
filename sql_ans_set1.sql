...
select S.product_id, P.product_name from product P inner join Sales S on P.product_id =S.product_id where S.product_id not in (select product_id from Sales where sale_date between '2019-04-01' and '2019-12-31');
...
#18.There is no primary key for this table, it may have duplicate rows.
...
select round((select count(*)  from delivery where order_date =customer_pref_delivery_date)/count(delivery_id) *100 ,2)as immidiate_percentage from delivery;
...
select ad_id,
ifnull(round(avg(case when action = "Clicked" then 1 when action = "Viewed" then 0 else null end) * 100,2),0)
as ctr
from Ads
